...
log.setLevel(logging.DEBUG)  # logging.INFO -> msg won't print
...
class BitField(object):
    ...
    def __init__(self, from_bit: int, to_bit: int) -> None:
        ...
        self.to_bit = to_bit
        mask = 0
        for i in range(self.to_bit + 1 - self.from_bit):
            mask = (mask << 1) | 1
        self.mask = mask
    ...
    def insert(self, value: int, word: int) -> int:
        """Insert value, which should be in the low order
         ...
         """
        # Bits 0..2 do not overlap bits 3..5
        max_value = 2 ** (self.to_bit - self.from_bit + 1) - 1
        if value > max_value:
            raise ValueError(f"value ({value}) is too large for bitfield of size {self.to_bit - self.from_bit + 1}")
